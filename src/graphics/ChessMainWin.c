...
MainWin* MainWindowCreate() {
	...
	res->nextToDiffWinTexture = SDL_CreateTextureFromSurface(res->mainRenderer,
			loadingSurface);
...
void MainWindowDraw(MainWin* src) {
	...
	if (src->nextToColorWinPressed == 1) {
		...
		SDL_RenderCopy(src->mainRenderer, src->backFromColorTexture, NULL,
				&backR);
		if (src->colorPressed > 0) {
			//printf("in draw entered if (src->colorPressed>0)\n ");
			SDL_RenderCopy(src->mainRenderer, src->startFromColorTexture, NULL,
					&startR);
		}
	}
	...
	if (src->nextToDiffWinPressed == 1) {
		...
		SDL_RenderCopy(src->mainRenderer, src->moderateTexture, NULL,
				&moderateR);
		...
		if (src->diffPressed > 0) {
			SDL_RenderCopy(src->mainRenderer, src->nextToColorWinTexture, NULL,
					&nextR);
		}
	}

	if (src->newGameFromMainMenuPressed == 1) {
		SDL_RenderCopy(src->mainRenderer, src->onePlayerTexture, NULL,
				&onePlayersR);
		SDL_RenderCopy(src->mainRenderer, src->twoPlayerTexture, NULL,
				&twoPlayersR);
		SDL_RenderCopy(src->mainRenderer, src->backFromNumPlayersTexture, NULL,
				&backR);
		if (src->onePlayerPressed == 1) {
			SDL_RenderCopy(src->mainRenderer, src->nextToDiffWinTexture, NULL,
					&nextR);
		} else {
			if (src->numPlayersPressed == 1) {
				SDL_RenderCopy(src->mainRenderer,
						src->startFromNumPlayersTexture, NULL, &startR);
			}
		}
	}
			...
			if (src->colorPressed > 0) {
				...
				if (isClickOnStart(event->button.x, event->button.y)) {
					...
					if (g->PLAYERS == 1 && g->USER_COLOR == 0) {
						printf("entered pc move place in main win\n");
